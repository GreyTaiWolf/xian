# 仙 V2.0（单机网页版）

AI 天道导演 × 确定性刷宝修真世界。**代码是物理法则，AI 是天道。**

V2.0 已完成战斗与秘境纵向升级：所有攻击统一经过“排队 → 前摇 → 位移/弹道 → 命中 → 收招”，伤害只在命中节点结算；怪物死亡保留退场演出，秘境改为三波 encounter、失败回滚、通关三选一奖励的完整闭环。桌面与移动端界面同步升级为黑金墨韵战斗 HUD。

当前工程正在扩展为：

> **网游式角色成长、刷宝、副本、势力与社交体验 + 可持续运行的 AI 动态世界。**

像素/几何地图与文字 UI 负责表现；人口、生态、市场、势力、NPC 知识和世界历史由确定性规则维护。AI 只能提出受限候选，不能直接改数值或存档。

## 运行

```bash
npm.cmd install          # 安装依赖（PowerShell 下用 npm.cmd 以绕过脚本策略）
npm.cmd run dev          # 开发服务器 → http://localhost:5173
npm.cmd run typecheck    # TypeScript 严格检查
npm.cmd run selftest     # 原玩法回归 + 动态世界内核自测
npm.cmd run build        # 类型检查 + 产物构建
```

只运行动态世界内核测试：

```bash
npm.cmd run worldsim:selftest
```

## 当前存档与世界时间

- 当前存档版本：`v10`。
- `GameState` 已正式包含 `simulation` 动态世界状态。
- v9 及更早存档会按迁移链升级；V2 会补齐秘境进度、自动技能与战斗速度设置，并继续按原世界种子、世界日归一动态模拟。
- 每个旧玩法世界日对应 24 个模拟小时；每日边界会同步推进经济、生态、市场与势力状态。
- 离线快进在运行时构造前完成，奖励、编年史和模拟世界在同一次状态提交中更新。
- 单次离线结算最多应用 720 个世界日，防止奖励膨胀和主线程长时间阻塞。
- 离线编年史模板使用可播种随机流，不再依赖 `Math.random()`。

## 文档

| 文件 | 内容 |
|---|---|
| `docs/game-design.md` | 总体设计（AI/代码分工、地图生成、天道机制） |
| `docs/phases.md` | P0~M4 分阶段内容设计 |
| `docs/ui-design.md` | UI 规范（调色板/布局/组件/交互） |
| `docs/ui-reference-gallery.md` | UI 视觉参考图库（概念图、可借鉴点与落地边界） |
| `docs/ai-world-simulation.md` | AI 动态世界架构、数据模型、Tick、存档与接入计划 |
| `docs/ai-prompt-contracts.md` | 游戏内 AI 提示词契约与结构化输出边界 |
| `ui/prototype.html` | 静态原型（UI 视觉基准，双击可预览） |

## 目录结构

```text
src/
  core/         框架核心：主循环、事件总线、状态容器、存档、RNG、配置表
  game/         现有角色、装备、任务与可存档数据
  simulation/   动态世界：经济、生态、势力、事件、存档同步、离线规划、AI 安全门
  world/        世界层：世界时钟、程序化格子世界
  render/       渲染层：Canvas 渲染器、摄像机、地形预览
  ui/           界面层：主题、面板、弹层、屏幕
  config/       配置表 JSON（数值驱动，改表不改码）
scripts/
  selftest.ts
  worldsim-selftest.ts
```

## 阶段状态

- [x] P0 整体框架 + UI 界面系统
- [x] M0 可玩闭环（移动/战斗/掉落/背包装备/升级/死亡）
- [x] M1 程序化地图（chunk 化无限世界 + value-noise 管线 + 等级梯度）
- [x] M2 天道 AI（世界导演/编年史/种子卡/NPC 对话/模板降级）
- [x] M3 内容纵深（宗门/城镇/秘境/境界突破/任务/合成/离线快进）
- [x] 借鉴改造（9 档品级+词条 / 地点行动列表 / 境界小阶段+心境悟性 / 文档纪律）
- [x] M4 打磨（动效/音效/成就/世界Boss首杀/新手引导/性能）
- [x] 借鉴二轮（行路异闻两选一+因果链 / 主线阶段卡 CTA）
- [x] WS-P0 动态世界内核（确定性 Tick / 经济生态 / 事件日志 / AI 安全门）
- [x] WS-P1 主存档与运行时接入（v9 迁移 / 世界日同步 / 离线快进 / 26 项动态世界断言）
- [x] V2.0 战斗纵切（确定性行动时间线 / 三技能 / 命中结算 / 死亡演出 / 三波秘境 / 奖励择一 / 响应式战斗 HUD）
- [ ] WS-P2 玩家可见世界闭环（城镇市场 / 怪物迁徙 / 动态任务 / 世界新闻 UI）
- [ ] WS-P3 重要 NPC 社会层（关系 / 知识 / 记忆 / 自然对话闭环）
